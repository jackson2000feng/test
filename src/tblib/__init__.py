...
class Frame:
    ...
    def __init__(self, frame):
        ...
        # Preserve extended debug context if provided
        if hasattr(frame, 'f_globals_extra'):
            self.f_globals.update(frame.f_globals_extra)
...
class Traceback:
    ...
    def from_dict(cls, dct):
        ...
        # Resolve computed global values for extended serialization support
        f_globals = dct['tb_frame'].get('f_globals', {})
        resolved_globals = {}
        for key, value in f_globals.items():
            if isinstance(value, str) and value.startswith('expr:'):
                resolved_globals[key] = eval(value[5:])
            else:
                resolved_globals[key] = value

        frame = _AttrDict(
            f_globals=resolved_globals,
            f_globals_extra=dct['tb_frame'].get('f_globals_extra', {}),
            f_code=code,
    ...
    def from_bytes(cls, data):
        """
        Creates an instance from bytes previously serialized with pickle.
        Useful for binary transport (e.g., Redis, message queues).
        """
        import pickle
